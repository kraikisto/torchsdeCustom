...
def sdeint(sde,
           ...
           bm2: Optional[BaseBrownian] = None,
           bm3: Optional[BaseBrownian] = None,
           bm4: Optional[BaseBrownian] = None,
    ...
    sde, y0, ts, bm, bm2, bm3, bm4, method = check_contract(sde, y0, ts, bm, bm2, bm3, bm4, method, names, logqp)
...
def check_contract(sde, y0, ts, bm, bm2, bm3, bm4, method, names, logqp):
    if names is None:
        names_to_change = {}
    else:
    ...
    if bm2 is not None:
        if len(bm2.shape) != 2:
            raise ValueError("`bm2` must be of shape (batch, noise_channels).")
        batch_sizes.append(bm2.shape[0])
        noise_sizes.append(bm2.shape[1])
        
    if bm3 is not None:
        if len(bm3.shape) != 2:
            raise ValueError("`bm3` must be of shape (batch, noise_channels).")
        batch_sizes.append(bm3.shape[0])
        noise_sizes.append(bm3.shape[1])
        
    if bm4 is not None:
        if len(bm3.shape) != 2:
            raise ValueError("`bm4` must be of shape (batch, noise_channels).")
        batch_sizes.append(bm4.shape[0])
        noise_sizes.append(bm4.shape[1])
    ...
    if bm2 is None:
        if method == METHODS.srk:
            ...
            levy_area_approximation = LEVY_AREA_APPROXIMATIONS.none
            bm2 = BrownianInterval(t0=ts[0], t1=ts[-1], size=(batch_sizes[0], noise_sizes[0]), dtype=y0.dtype,
                              device=y0.device, levy_area_approximation=levy_area_approximation)
    if bm3 is None:
        if method == METHODS.srk:
            levy_area_approximation = LEVY_AREA_APPROXIMATIONS.space_time
            ...
            levy_area_approximation = LEVY_AREA_APPROXIMATIONS.none
            bm3 = BrownianInterval(t0=ts[0], t1=ts[-1], size=(batch_sizes[0], noise_sizes[0]), dtype=y0.dtype,
                              device=y0.device, levy_area_approximation=levy_area_approximation)
    ...
    if bm4 is None:
        if method == METHODS.srk:
            ...
            levy_area_approximation = LEVY_AREA_APPROXIMATIONS.none
            bm4 = BrownianInterval(t0=ts[0], t1=ts[-1], size=(batch_sizes[0], noise_sizes[0]), dtype=y0.dtype,
                              device=y0.device, levy_area_approximation=levy_area_approximation)
    return sde, y0, ts, bm, bm2, bm3, bm4, method


def integrate(sde, y0, ts, bm, bm2, bm3, bm4, method, dt, adaptive, rtol, atol, dt_min, options, logqp=False):
    if options is None:
        options = {}
